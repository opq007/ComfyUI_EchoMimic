...
class WanTransformerAudioMask3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def from_pretrained(
        ...
        if os.path.isfile(config_file):
            ...
            # the correct Wan2.1-Fun-1.3B structure (in_dim=16, dim=2048,
            # ffn_dim=8192, text_dim=4096, num_heads=16), and the actual flash
            # weights are applied afterwards via load_state_dict(strict=False),
            # so falling back to defaults is safe instead of hard-failing.
            ...
            config = {}
            # EchoMimic flash-pro is an image-to-video (i2v) model. The
            # i2v/t2v choice selects the cross-attention block type which maps
            # to different weight keys, so it must match the checkpoint. The
            # Wan2.1-Fun-1.3B structure below is the vanilla i2v default.
